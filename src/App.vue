          ...
          <ul class="nav flex-column">
            <li class="nav-item">
              ...
              <router-link class="nav-link" :class="{ active: $route.name === 'orders' }" :to="{ name: 'orders' }"
                @click="clickMenuOption" v-if="userStore.user && userStore.user.type == 'EC'">
                <i class="bi bi-egg-fried"></i>
                Chef:Items to prepare
              </router-link>
              <router-link class="nav-link" :class="{ active: $route.name == 'Order' }" :to="{ name: 'Order' }"
                @click="clickMenuOption" v-if="userStore.userId == -1 || userStore.user.type == 'C'">
                <i class="bi bi-list"></i>
                Order ({{ orderStore.totalProducts }} Products)
              </router-link>
              <div v-if="userStore.userId != -1 && userStore.user.type == 'EM'">
                <router-link class="nav-link" :class="{ active: $route.name === 'Users' }" :to="{ name: 'Users' }"
                  @click="clickMenuOption">
                  Users
                </router-link>
              </div>
              <router-link class="nav-link" :class="{ active: $route.name === 'employeeOrders' }"
                :to="{ name: 'employeeOrders' }" @click="clickMenuOption"
                v-if="userStore.user && userStore.user.type == 'ED'">
                <i class="bi bi-bag-check"></i>
                Employee:Orders to prepare
              </router-link>
              <router-link class="nav-link" :class="{ active: $route.name == 'ordersManager' }"
                :to="{ name: 'ordersManager' }" @click="clickMenuOption"
